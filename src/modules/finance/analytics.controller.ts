...
export const analyticsController = {
  async getMetrics(req: Request, res: Response) {
    try {
      const [invoiceAgg, overdueCount, pendingPayouts, customerCount] = await Promise.all([
        ...
        prisma.invoice.count({ where: { status: 'overdue' as any } }),
  ...
  async getPaymentMethodStats(req: Request, res: Response) {
    try {
      const { dateFrom, dateTo } = req.query as { dateFrom?: string; dateTo?: string };
      const from = dateFrom ? new Date(dateFrom) : undefined;
      const to = dateTo ? new Date(dateTo) : undefined;
      ...
      for (const p of payments) {
        ...
        if (!totals[key]) totals[key] = { count: 0, totalAmount: 0 };
        totals[key].count += 1;
  ...
  async getAgingReport(req: Request, res: Response) {
    try {
      ...
      const now = new Date();
      const byCustomer: Record<string, { name: string; current: number; thirtyDays: number; sixtyDays: number; ninetyDays: number; total: number }> = {};
      ...
      for (const inv of invoices) {
        ...
        if (diffDays <= 0) byCustomer[customerId].current += amount;
        else if (diffDays <= 30) byCustomer[customerId].thirtyDays += amount;
        else if (diffDays <= 60) byCustomer[customerId].sixtyDays += amount;
        else byCustomer[customerId].ninetyDays += amount;
