...
import { AlertTriangle, DollarSign, MapPin, Package, Box, Plus, Trash2 } from 'lucide-react';
import type { Shipment, Currency, PackageType, ServiceType, DraftShipment } from '@/types/shipment';
...
export type ShipmentFormMode = 'create' | 'resume-draft' | 'view' | 'edit';
...
export const ShipmentForm: React.FC<ShipmentFormProps> = ({
  ...
  // Load existing shipment data for view/edit/resume-draft modes
  useEffect(() => {
    const loadExistingData = async () => {
      ...
      try {
        ...
        if (data) {
          setExistingShipment(data);
          
          // Pre-fill form values
          if (data.sender) {
            Object.keys(data.sender).forEach((key) => {
              setValue(`sender.${key}` as any, (data.sender as any)[key]);
            });
          }
          if (data.recipient) {
            Object.keys(data.recipient).forEach((key) => {
              setValue(`recipient.${key}` as any, (data.recipient as any)[key]);
            });
          }
          if (data.packageType) setValue('packageType', data.packageType);
          if (data.serviceType) setValue('serviceType', data.serviceType);
          if (data.packageDetails) {
            setValue('packageDetails', data.packageDetails);
          }
          if (data.commodities && data.commodities.length > 0) {
            // Clear existing and add new
            remove();
            data.commodities.forEach((item: any) => append(item));
          }
          if (data.options) {
            setValue('options', data.options);
          }
          if (data.specialInstructions) setValue('specialInstructions', data.specialInstructions);
          if (data.paymentType) setPaymentType(data.paymentType);
          if (data.accountNumber) setManualAccountNumber(data.accountNumber);
          if (data.chargesInformation) {
            setValue('chargesInformation', data.chargesInformation);
            setChargesCurrency(data.chargesInformation.currency);
          }
          if (data.totalWeight || data.totalValue || data.totalCurrency) {
            // These are computed, don't set them directly
          }
        }
      } catch (error) {
        ...
        toast({
          ...
          description: 'Could not load shipment information.',
          variant: 'destructive',
        });
      } finally {
        setLoading(false);
  ...
  const handleSaveAsDraft = async (data: ShipmentFormData) => {
    try {
      const payload = {
        ...data,
        paymentType,
      ...
      toast({
        title: 'Draft Saved',
        description: 'Your shipment draft has been saved successfully.',
        variant: 'default',
      });

      // If we were in create mode, redirect to resume mode with the new draft ID
      if (isCreateMode && result?.id) {
        window.history.pushState({}, '', `/dashboard/shipments/create?draftId=${result.id}`);
      }
    } catch (error: any) {
  ...
  const handleFinalize = async (data: ShipmentFormData) => {
    ...
    try {
      ...
      if (isResumeDraftMode && draftId) {
        ...
        const backendShipmentData = {
          ...
          originCountry: data.sender?.country,
          destinationAddress: data.recipient?.address1,
          destinationCompany: data.recipient?.company,
          ...
          chargesAmount: Number(data.chargesInformation?.amount || 0),
      ...
      if (result) {
        ...
        const awbNumberForPdf = result.reference || result.awbNumber;
        ...
        const invoicePdfBlob = await generateInvoicePDF({
          ...
          awbReferences: [
            {
              ...
              valueETB: result.chargesInformation?.amount || 0,
            },
          ],
          ...
          subtotal: result.chargesInformation?.amount || 0,
          vatPercentage: 15,
          vatAmount: (result.chargesInformation?.amount || 0) * 15 / 100,
          total: (result.chargesInformation?.amount || 0) * 1.15,
          paymentInfo: {
      ...
      console.error('Failed to create/finalize shipment:', error);
      toast({
        title: 'Error',
  ...
  const handlePreviewAWB = async (data: ShipmentFormData) => {
    try {
      const { generateAwbPdf } = await import('@/shared/utils/generateAwb');
    ...
    <form className="space-y-8">
          ...
          <div className="border-t pt-5">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="space-y-2">
                ...
                <Select 
                  value={chargesCurrency} 
                  onValueChange={(value) => setChargesCurrency(value as Currency)}
                  disabled={isReadOnly}
                >
            ...
            <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="flex items-center space-x-2">
                  <Checkbox id="insurance" {...register('options.insurance')} disabled={isReadOnly} />
                  <Label htmlFor="insurance">Insurance Coverage</Label>
                </div>
                <div className="flex items-center space-x-2">
                  <Checkbox id="signature" {...register('options.signature')} disabled={isReadOnly} />
                  <Label htmlFor="signature">Signature Required</Label>
                </div>
                <div className="flex items-center space-x-2">
                  <Checkbox id="fragile" {...register('options.fragile')} disabled={isReadOnly} />
                  <Label htmlFor="fragile">Fragile Handling</Label>
                </div>
                <div className="flex items-center space-x-2">
                  <Checkbox id="saturday" {...register('options.saturday')} disabled={isReadOnly} />
      ...
      {!isReadOnly && (
        <div className="flex justify-end space-x-4">
          ...
          {(isCreateMode || isResumeDraftMode) && (
            <>
              <Button
                type="button"
                variant="outline"
              ...
              <Button
                type="button"
                variant="outline"
                onClick={handleSubmit(handlePreviewAWB)}
              >
              ...
              <Button
                ...
                disabled={loading}
              >
                {isResumeDraftMode ? 'Finalize Shipment' : 'Create Shipment'}
              </Button>
            </>
          )}
          {isEditMode && (
