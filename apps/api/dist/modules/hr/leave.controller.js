...
const createLeaveSchema = zod_1.z.object({
    ...
    backupEmployeeId: zod_1.z.string().optional(),
});
const updateLeaveStatusSchema = zod_1.z.object({
    status: zod_1.z.enum(['APPROVED', 'REJECTED']),
    comments: zod_1.z.string().optional(),
});
const updateLeaveBalanceSchema = zod_1.z.object({
    annualLeave: zod_1.z.number().min(0),
    sickLeave: zod_1.z.number().min(0),
    personalLeave: zod_1.z.number().min(0),
});
class LeaveController {
    constructor() {
         ...
         */
        this.createLeaveRequest = async (req, res) => {
            try {
                ...
                switch (data.leaveType) {
                    case 'ANNUAL':
                        balanceType = 'annualLeave';
                        break;
                    case 'SICK':
                        balanceType = 'sickLeave';
                        break;
                    case 'OTHER':
                        balanceType = 'personalLeave';
                        break;
                ...
                const defaultHr = await prisma_1.default.user.findFirst({
                    where: { role: { in: ['hr_manager', 'hr_staff'] } },
                    include: { employee: true }
                });
                const leaveRequest = await prisma_1.default.leave.create({
                    data: {
                ...
                if (employee.managerId) {
                    ...
                    await emailService_1.emailService.sendNotificationEmail(employee.managerId, 'LEAVE_REQUEST', {
                        leaveType: data.leaveType,
                        duration: totalDays,
         ...
         */
        this.getLeaveRequests = async (req, res) => {
            try {
         ...
         */
        this.getLeaveRequestById = async (req, res) => {
            try {
                const { id } = req.params;
                const userId = req.user?.sub;
         ...
         */
        this.updateManagerApproval = async (req, res) => {
            try {
                const { id } = req.params;
                const userId = req.user?.sub;
                ...
                await emailService_1.emailService.sendNotificationEmail(leave.employee.userId, 'LEAVE_STATUS', {
                    status,
                    comments,
         ...
         */
        this.updateHrApproval = async (req, res) => {
            try {
                const { id } = req.params;
                const userId = req.user?.sub;
                const { status, comments } = updateLeaveStatusSchema.parse(req.body);
                const hr = await prisma_1.default.employee.findUnique({
                    where: { userId },
                    include: { user: true },
                });
                if (!hr)
                ...
                const updatedLeave = await prisma_1.default.$transaction(async (tx) => {
       ...
       */
        this.getEmployeeLeaveBalanceById = async (req, res) => {
            try {
         ...
         */
        this.updateEmployeeLeaveBalance = async (req, res) => {
            try {
                const { employeeId } = req.params;
                const data = updateLeaveBalanceSchema.parse(req.body);
                let existing = null;
                try {
                    existing = await prisma_1.default.leaveBalance?.findUnique({ where: { employeeId } });
                }
                catch (err) {
                ...
                let updated = null;
                if (existing) {
                    updated = await prisma_1.default.leaveBalance?.update({ where: { employeeId }, data });
                }
                else {
                    try {
                        updated = await prisma_1.default.leaveBalance?.create({ data: { employeeId, ...data } });
                    }
                    catch (err) {
         ...
         */
        this.deleteLeaveRequest = async (req, res) => {
            try {
                const { id } = req.params;
                const userId = req.user?.sub;
                const leave = await prisma_1.default.leave.findUnique({
                ...
                await prisma_1.default.leave.delete({ where: { id } });
         ...
         */
        this.getLeaveApprovers = async (req, res) => {
            try {
                const userId = req.user?.sub;
                if (!userId) {
                ...
                const managers = await prisma_1.default.employee.findMany({
                    where: {
                        OR: [
                            { user: { role: 'hr_manager' } },
                            { subordinates: { some: {} } },
                ...
                const hrStaff = await prisma_1.default.user.findMany({
                    where: {
                ...
                const backupEmployees = await prisma_1.default.employee.findMany({
                    where: {
                        AND: [
         ...
         * GET /hr/leave-types
         */
        this.getLeaveTypes = async (req, res) => {
            try {
                const userId = req.user?.sub;
                if (!userId) {
                    return res.status(401).json({ error: 'Authentication required' });
                ...
                const leaveTypesWithBalance = await Promise.all(leaveTypesConfig.map(async (type) => {
                    const usedDays = await this.calculateUsedLeaveDays(employee.id, type.id, currentYear);
    ...
    async calculateUsedLeaveDays(employeeId, leaveType, year) {
        try {
            const where = {
                employeeId,
                leaveType,
            ...
            const usedLeaves = await prisma_1.default.leave.findMany({
                where,
    ...
    mapBalanceTypeToLeaveType(balanceType) {
        switch (balanceType) {
    ...
    async getEmployeeLeaveBalance(employeeId, balanceType, year) {
        try {
            let dbBalance = null;
            try {
                dbBalance = await prisma_1.default.leaveBalance?.findUnique({
                    where: { employeeId },
                });
            }
            catch (err) {
    ...
    async getCurrentYearLeaveBalance(employeeId) {
        const currentYear = new Date().getFullYear();
