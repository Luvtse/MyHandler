...
const createEmployeeSchema = zod_1.z.object({
    ...
    employmentStatus: zod_1.z.enum(['ACTIVE', 'ON_LEAVE', 'TERMINATED', 'PROBATION', 'REMOTE']).default('ACTIVE'),
...
function parsePage(input, fallback) {
...
async function getAnnualLeaveSummary(employeeId) {
    const now = new Date();
    ...
    const [usedAgg, pendingCount] = await Promise.all([
        prisma_1.default.leave.aggregate({
            where: {
                employeeId,
                leaveType: 'ANNUAL',
                status: 'APPROVED',
...
exports.employeeController = {
    ...
    async getColleagues(req, res) {
        try {
            ...
            const colleagues = await prisma_1.default.employee.findMany({
                where: {
                    id: { not: employee.id },
                    OR: [
    ...
    async getAll(req, res) {
        try {
            ...
            const departmentId = normalizeString(req.query.department);
            const teamId = normalizeString(req.query.team);
    ...
    async exportCsv(req, res) {
        try {
            ...
            const status = normalizeString(req.query.status);
            const where = buildEmployeeWhere({ search, departmentId, teamId, status });
            const rows = await prisma_1.default.employee.findMany({
                where,
                select: employeeSelect,
                orderBy: { createdAt: 'desc' },
                take: 5000,
