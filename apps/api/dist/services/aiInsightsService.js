...
exports.getCeoInsights = getCeoInsights;
exports.getCfoInsights = getCfoInsights;
exports.getCooInsights = getCooInsights;
exports.getCmoInsights = getCmoInsights;
/**
 * AI Insights Service
...
async function getCeoInsights() {
    const insights = [];
    const yearStart = new Date(new Date().getFullYear(), 0, 1);
    const [ytdRevenue, prevYearRevenue, totalDelivered, totalShipments, recentClients, prevClients,] = await Promise.all([
        ...
        prisma_1.default.shipment.count({
            where: {
                status: { in: ['DELIVERED_SUCCESSFULLY', 'DELIVERY_CONFIRMED', 'SIGNATURE_OBTAINED'] },
...
async function getCfoInsights() {
    ...
    const overdueAmount = invoicesOverdue.reduce((sum, inv) => sum + inv.totalAmount, 0);
    ...
    if (overdueAmount > 0) {
        insights.push({
            ...
            confidence: 0.96,
            severity: overdueAmount > 500000 ? 'high' : 'medium',
...
async function getCooInsights() {
    const insights = [];
    const [totalVehicles, idleVehicles, stockoutItems, delayedShipments, totalActive] = await Promise.all([
        ...
        prisma_1.default.shipment.count({
            where: {
                status: {
                    notIn: ['DELIVERED_SUCCESSFULLY', 'DELIVERY_CONFIRMED', 'CANCELLED', 'RETURNED_TO_SENDER', 'LOST_EXCEPTION', 'SIGNATURE_OBTAINED'],
                },
            },
        }),
    ]);
...
async function getCmoInsights() {
    const insights = [];
    const [totalQuotations, wonQuotations, activeClients, totalClients] = await Promise.all([
