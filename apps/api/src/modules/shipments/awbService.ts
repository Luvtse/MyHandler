...
export const awbGenerationService = {
  async generateNextAwb(tx: any): Promise<string> {
    try {
      const now = new Date();
  ...
  async getNextSequence(tx: any): Promise<string> {
    const now = new Date();
  ...
  calculateChecksum(awbBase: string): string {
    let sum = 0;
  ...
  validateAWB(awb: string): { isValid: boolean; error?: string } {
  ...
  async getAWBStats() {
    ...
    const [totalCount, monthlyCount, lastShipment] = await Promise.all([
      ...
      prisma.shipment.findFirst({ orderBy: { createdAt: 'desc' }, select: { createdAt: true } }),
    ]);
