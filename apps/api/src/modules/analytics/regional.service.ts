...
export interface RegionalMetrics {
  region: string;
  shipments: number; // created today
  revenue: number; // ETB, month-to-date
  activeClients: number; // distinct customers with shipments in last 90 days
...
function regionCityFilter(region: string) {
  const norm = normalizeRegion(region);
  const aliases = REGION_ALIASES[norm.replace(/\s+/g, '_')] ?? [norm];

  const likeAny = (field: string) =>
    aliases.map((a) => ({ [field]: { contains: a } }));
...
function buildInsights(m: RegionalMetrics): RegionalInsight[] {
  ...
  if (m.fleetUtilization > 90) {
    insights.push({
      ...
      severity: 'medium',
      recommendation:
        'Request additional vehicles from central pool or rebalance routes.',
    });
  }
...
export async function getRegionalData(regionParam: string): Promise<RegionalData> {
  ...
  const [
    ...
    prisma.shipment.count({ where: { ...cityFilter, createdAt: { gte: today } } }),
    ...
    prisma.shipment.findMany({
      where: { ...cityFilter, status: { in: DELIVERED_STATUSES as any[] } },
    ...
    prisma.shipment.count({
      where: { status: { in: DELIVERED_STATUSES as any[] }, updatedAt: { gte: d90 } },
    }),
    prisma.shipment.count({
      where: { status: { in: TERMINAL_STATUSES as any[] }, updatedAt: { gte: d90 } },
    }),
    prisma.shipment.aggregate({
      _avg: { chargesAmount: true },
  ...
  for (const row of fulfillmentRows) {
    const durationH =
      (new Date(row.updatedAt).getTime() - new Date(row.createdAt).getTime()) / 3_600_000;
    if (durationH >= 0) fulfillmentSumHours += durationH;
  }

  const onTimeDelivery =
    totalTerminalLast90 > 0 ? round1((deliveredLast90 / totalTerminalLast90) * 100) : 0;
  const avgFulfillmentTime =
    fulfillmentRows.length > 0 ? round1(fulfillmentSumHours / fulfillmentRows.length) : 0;
  const [totalVehicles, activeVehicles] = vehicleCounts;
  const fleetUtilization =
    totalVehicles > 0 ? round1((activeVehicles / totalVehicles) * 100) : 0;
  ...
  return {
    ...
    benchmarks: {
      nationalOnTimeDelivery:
        nationalTerminal > 0 ? round1((nationalDelivered / nationalTerminal) * 100) : 0,
